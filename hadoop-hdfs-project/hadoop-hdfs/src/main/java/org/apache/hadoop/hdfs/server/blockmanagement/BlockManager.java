...
public class BlockManager {
  ...
  /** Replication thread. */
  // ReplicationMonitor，后台线程，专门监控有没有要复制的block
   ...
   * 
   * 这个里面存放的都是那些block的副本数量不充足的block
   * 
   */
  ...
  void removeBlocksAssociatedTo(final DatanodeDescriptor node) {
    ...
    // 这个datanode遍历下他里面有的block，对每个block都直接删除
    while(it.hasNext()) {
  ...
  int computeReplicationWork(int blocksToProcess) {
    ...
    try {
      ...
      // 从neededRepliations里面，待复制的任务队列，里面存放了要复制的block
  ...
  int computeReplicationWorkForBlocks(List<List<Block>> blocksToReplicate) {
    ...
    // 到此为止，对每个要复制的block，第一是选择了一个source datanode
    // 第二是选择了要复制过去的目标的datanode
      ...
      for(ReplicationWork rw : work){
        ...
        synchronized (neededReplications) {
          ...
          if(numEffectiveReplicas + targets.length >= requiredReplication) {
        	// 给搞完了以后，分配好了复制任务了，已经加入了source Datanode的复制任务队列里面去
        	// 直接从neededReplications中移除掉对应的block就可以了
            neededReplications.remove(block, priority); // remove from neededReplications
            neededReplications.decrementReplicationIndex(priority);
          }
        }
  ...
  public DatanodeStorageInfo[] chooseTarget4NewBlock(final String src,
    ...
    // 核心在这里
    // BlockPlacementPolicy组件，是专门负责决定一个block是放在哪里的
  ...
  throws IOException {
    ...
    // 每次你上报完block之后，就会处理under replication / over replication
    // under replication，就是说副本数量不足3；over replication，就是说副本数量超过3
  ...
  public void removeStoredBlock(Block block, DatanodeDescriptor node) {
    ...
    assert (namesystem.hasWriteLock());
    {
      // blocksMap就是存放datanode和block的映射关系的
      // 删除关联关系
      ...
      // be-replicated list.
      // 就可以看到，如果发现说一个datanode宕机，导致一个block副本数量变少了
      // 在这里，就会判断将block放入一个需要复制副本的列表中
  ...
  private void processAndHandleReportedBlock(
    ...
    for (StatefulBlockInfo b : toUC) { 
      // block under construction，就是还停留在传输过程中，一个block对应了N多个packet
      addStoredBlockUnderConstruction(b, storageInfo);
    }
    long numBlocksLogged = 0;
    // 如果是finalized状态，此时一定会将block加入toAdd列表中
    for (BlockInfo b : toAdd) {
  ...
  private void updateNeededReplications(final Block block,
    ...
    try {
      ...
      int curExpectedReplicas = getReplication(block);
      // 判断一下，当前这个block还存在的datanode上的副本数量（可能就只有2个副本）
      // 还有期望的副本因子数量（期望3个副本）
      // 判断一下当前的这个block副本数量是否不充足，是否需要复制
