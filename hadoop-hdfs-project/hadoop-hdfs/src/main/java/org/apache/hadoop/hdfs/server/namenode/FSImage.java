...
public class FSImage implements Closeable {
  ...
  private boolean loadFSImage(FSNamesystem target, StartupOption startOpt,
    ...
    for (int i = 0; i < imageFiles.size(); i++) {
      try {
    	// 在这里的话呢，他就会去加载FSImageFile，加载到内存里来
  ...
  public void initEditLog(StartupOption startOpt) throws IOException {
    ...
    if (!HAUtil.isHAEnabled(conf, nameserviceId)) {
      ...
      // 如果是HA状态的话，应该是调用这个方法，这个方法会去触发底层的JournalSet里的JournalManager的初始化
      // 我们读的源码是2.6版本，这个版本，从源码可以看到，有些hadoop的版本，人家其实在写edits log的时候
      // 需要的JournalManager，就是在namenode刚启动的时候初始化的
  ...
  private long loadEdits(Iterable<EditLogInputStream> editStreams,
    ...
    try {    
      ...
      for (EditLogInputStream editIn : editStreams) {
        ...
        try {
          // 核心的逻辑应该是在这里
          // FSEditLogLoader组件，他依赖了底层的EditLogInputStream去拉取数据
   ...
   * 
   * 用屁股想想，就知道了，将内存里的数据组织成一定的格式，然后用二进制的方式写入到磁盘文件里去
   * 一般人无法打开文件就阅读的，他应该是有自己的特殊的序列化的格式
   * 
   */
  ...
  private synchronized void saveFSImageInAllDirs(FSNamesystem source,
    ...
    try {
      ...
      // 如果你写了一个新的fsimage文件之后，你就可以清理掉之前的一些edits log文件和checkpoint文件
      purgeOldStorage(nnf);
    } finally {
   ...
   * 他在写一个fsimage文件的时候，可能是一个别的什么名字，写完了以后，再改一下名字
   */
